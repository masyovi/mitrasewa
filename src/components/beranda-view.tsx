...
const equipmentGallery = [
  {
    name: "Scaffolding",
    desc: "Rangka baja kokoh untuk penopang pekerjaan konstruksi bangunan bertingkat",
    icon: "🏗️",
    color: "from-emerald-500 to-teal-600",
  },
  {
    name: "Mesin Molen",
    desc: "Pengaduk beton otomatis untuk campuran adukan yang merata dan cepat",
    icon: "🔄",
    color: "from-amber-500 to-orange-600",
  },
  {
    name: "Mesin Stamper",
    desc: "Pemadat tanah profesional untuk persiapan lahan dan fondasi",
    icon: "⚡",
    color: "from-blue-500 to-indigo-600",
  },
  {
    name: "Catwalk",
    desc: "Pijakan scaffolding aman untuk area kerja di ketinggian",
    icon: "🪜",
    color: "from-violet-500 to-purple-600",
  },
  {
    name: "Joint Pin",
    desc: "Penghubung vertikal yang mengunci frame scaffolding dengan kokoh",
    icon: "🔗",
    color: "from-rose-500 to-pink-600",
  },
  {
    name: "U Head",
    desc: "Penopang balok di atas scaffolding untuk menahan beban lantai kerja",
    icon: "🛡️",
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-8 space-y-8">
          ...
          <Card className="border-0 shadow-md bg-white dark:bg-gray-900 overflow-hidden card-elevated">
            <CardContent className="p-5 sm:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex items-start gap-3 hover-lift rounded-xl p-2 -m-2">
                  ...
                  <div>
                    ...
                    <a href="https://wa.me/6285185924243" target="_blank" rel="noopener noreferrer" className="text-sm text-emerald-600 font-medium mt-0.5 hover:text-emerald-700 transition-colors">
                      0851-8592-4243
                    </a>
              ...
              <a
                ...
                className="mt-4 flex items-center justify-center gap-2 w-full h-11 bg-[#25D366] hover:bg-[#20BD5A] text-white font-semibold rounded-xl transition-all btn-press animate-fade-in-up"
              >
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {equipmentGallery.map((eq, idx) => (
              <div
                key={idx}
                className={`relative overflow-hidden rounded-xl p-4 sm:p-5 text-center cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-xl group ${idx % 2 === 0 ? 'sm:mt-0' : 'sm:mt-6 lg:mt-0'}`}
                style={{ animationDelay: `${idx * 0.1}s` }}
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${eq.color} opacity-10 group-hover:opacity-20 transition-opacity duration-300`} />
                <div className="relative z-10">
                  <span className="text-3xl sm:text-4xl block mb-2 group-hover:scale-125 transition-transform duration-300">
                    {eq.icon}
                  </span>
                  <p className="font-bold text-gray-900 dark:text-gray-100 text-xs sm:text-sm">
                    {eq.name}
                  </p>
                  <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-1 leading-snug hidden sm:block">
                    {eq.desc}
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li><span className="text-xs text-white/60 hover:text-white/90 transition-colors cursor-pointer flex items-center gap-1.5"><Wrench className="w-3.5 h-3.5" />Perawatan Alat</span></li>
              </ul>
            </div>
